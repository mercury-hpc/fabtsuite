...
static char *completion_flags_to_string(const uint64_t flags, char *const buf,
...
static void warnv_about_ofi_ret_impl(int ret, const char *fn, int lineno,
...
static void warn_about_ofi_ret_impl(int ret, const char *fn, int lineno,
                                    const char *fmt, ...) {
  va_list ap;
...
static void bailout_for_ofi_ret_impl(int ret, const char *fn, int lineno,
                                     const char *fmt, ...) {
  va_list ap;
...
static int minsize(size_t l, size_t r) { return (l < r) ? l : r; }

static inline bool size_is_power_of_2(size_t size) {
  return ((size - 1) & size) == 0;
}

static fifo_t *fifo_create(size_t size) {
 ...
 */
static inline bool fifo_eoget(const fifo_t *f) {
 ...
 */
static inline bool fifo_eoput(const fifo_t *f) {
 ...
 * will be true.
 */
static inline void fifo_get_close(fifo_t *f) {
  assert(!fifo_eoget(f));
  f->closed = f->removals;
}
 ...
 * will be true.
 */
static inline void fifo_put_close(fifo_t *f) {
  assert(!fifo_eoput(f));
  f->closed = f->insertions;
}

static void fifo_destroy(fifo_t *f) { free(f); }

/* See `fifo_get`: this is a variant that does not respect the close
 * position.
 */
static inline bufhdr_t *fifo_alt_get(fifo_t *f) {
 ...
 * FIFO.
 */
static inline bufhdr_t *fifo_get(fifo_t *f) {
  if (fifo_eoget(f))
    return NULL;
 ...
 * position.
 */
static inline bool fifo_alt_empty(fifo_t *f) {
 ...
 * removing it.
 */
static inline bufhdr_t *fifo_peek(fifo_t *f) {
  assert(f->insertions >= f->removals);
 ...
 * position.
 */
static inline bool fifo_alt_full(fifo_t *f) {
 ...
 * position.
 */
static inline bool fifo_alt_put(fifo_t *f, bufhdr_t *h) {
 ...
 */
static inline bool fifo_put(fifo_t *f, bufhdr_t *h) {
  if (fifo_eoput(f))
    return false;
...
static bufhdr_t *buflist_get(buflist_t *bl) {
  if (bl->nfull == 0)
    return NULL;
...
static bool buflist_put(buflist_t *bl, bufhdr_t *h) {
...
static bool session_init(session_t *s, cxn_t *c, terminal_t *t) {
  memset(s, 0, sizeof(*s));
...
static void keysource_init(keysource_t *s) { memset(s, 0, sizeof(*s)); }

static uint64_t keysource_next(keysource_t *s) {
  if (s->next_key % 256 == 0) {
    s->next_key =
        atomic_fetch_add_explicit(&next_key_pool, 256, memory_order_relaxed);
  }

  return s->next_key++;
}

static bufhdr_t *buf_alloc(size_t paylen) {
  bufhdr_t *h;
...
static void buf_free(bufhdr_t *h) { free(h); }

static bytebuf_t *bytebuf_alloc(size_t paylen) {
  return (bytebuf_t *)buf_alloc(paylen);
}

static fragment_t *fragment_alloc(void) {
  bufhdr_t *h;
  fragment_t *f;
...
static progbuf_t *progbuf_alloc(void) {
  bufhdr_t *h;
  progbuf_t *pb;
...
static vecbuf_t *vecbuf_alloc(void) {
  bufhdr_t *h;
  vecbuf_t *vb;
...
static int buf_mr_reg(struct fid_domain *dom, uint64_t access, uint64_t key,
                      bufhdr_t *h) {
  int rc;
  bytebuf_t *b = (bytebuf_t *)h;

  rc = fi_mr_reg(dom, b->payload, h->nallocated, access, 0, key, 0, &h->mr,
                 NULL);
...
static int buf_mr_dereg(bufhdr_t *h) { return fi_close(&h->mr->fid); }

static void vecbuf_free(vecbuf_t *vb) { buf_free(&vb->hdr); }

static bool worker_paybuflist_replenish(worker_t *w, uint64_t access,
                                        buflist_t *bl) {
  size_t i, paylen;
  int rc;
  ...
  for (paylen = 0, i = bl->nfull; i < ntofill; i++) {
    ...
    if (!global_state.reregister &&
        (rc = buf_mr_reg(global_state.domain, access, keysource_next(&w->keys),
                         &buf->hdr)) != 0) {
...
static bytebuf_t *worker_payload_txbuf_get(worker_t *w) {
  bytebuf_t *b;
...
static bytebuf_t *worker_payload_rxbuf_get(worker_t *w) {
  bytebuf_t *b;
...
static size_t fibonacci_iov_setup(void *_buf, size_t len, struct iovec *iov,
                                  size_t niovs) {
  char *buf = _buf;
  ssize_t i;
  struct fibo {
  ...
  for (i = 0; len > 0 && i < niovs - 1; i++) {
    ...
    len -= iov[i].iov_len;
    buf += iov[i].iov_len;
    state = (struct fibo){.prev = state.curr, .curr = state.prev + state.curr};
  }
  if (len > 0) {
    iov[i].iov_len = len;
    iov[i].iov_base = buf;
 ...
 */
static int mr_regv_all(struct fid_domain *domain, const struct iovec *iov,
                       size_t niovs, size_t maxsegs, uint64_t access,
                       uint64_t offset, keysource_t *keys, uint64_t flags,
                       struct fid_mr **mrp, void **descp, uint64_t *raddrp,
                       void *context) {
  int rc;
  ...
  for (nleftover = niovs, i = 0; i < nregs;
    ...
    rc = fi_mr_regv(domain, iov, nsegs, access, offset, keysource_next(keys),
                    flags, &mr, context);
...
static bufhdr_t *rxctl_complete(rxctl_t *rc, const completion_t *cmpl) {
  bufhdr_t *h;
  ...
  if ((h = fifo_get(rc->posted)) == NULL) {
    errx(EXIT_FAILURE, "%s: received a message, but no Rx was posted",
         __func__);
    return NULL;
  }
...
static void rxctl_post(cxn_t *c, rxctl_t *ctl, bufhdr_t *h) {
  int rc;
...
static void fifo_cancel(struct fid_ep *ep, fifo_t *posted) {
...
static void rxctl_cancel(struct fid_ep *ep, rxctl_t *ctl) {
  fifo_cancel(ep, ctl->posted);
}

static void txctl_cancel(struct fid_ep *ep, txctl_t *ctl) {
  fifo_cancel(ep, ctl->posted);
}
 ...
 * irrecoverable error.
 */
static int txctl_complete(txctl_t *tc, const completion_t *cmpl) {
  bufhdr_t *h;
...
static void txctl_transmit(cxn_t *c, txctl_t *tc) {
  bufhdr_t *h;
  ...
  while ((h = fifo_peek(tc->ready)) != NULL && !fifo_full(tc->posted)) {
    const int rc = fi_sendmsg(
        c->ep,
        &(struct fi_msg){
            .msg_iov = &(struct iovec){.iov_base = ((bytebuf_t *)h)->payload,
                                       .iov_len = h->nused},
            .desc = h->desc,
            .iov_count = 1,
            .addr = c->peer_addr,
...
static loop_control_t rcvr_start(worker_t *w, rcvr_t *r,
                                 fifo_t *ready_for_cxn) {
 ...
 */
static loop_control_t source_trade(terminal_t *t, fifo_t *ready,
                                   fifo_t *completed) {
 ...
 * bytes.
 */
static loop_control_t sink_trade(terminal_t *t, fifo_t *ready,
                                 fifo_t *completed) {
  sink_t *s = (sink_t *)t;
  bufhdr_t *h;
...
static bool progbuf_is_wellformed(progbuf_t *pb) {
 ...
 * irrecoverable error.
 */
static int rcvr_progress_rx_process(rcvr_t *r, const completion_t *cmpl) {
  progbuf_t *pb;
 ...
 */
static int rcvr_cq_process(rcvr_t *r) {
  ...
  if (ncompleted == -FI_EAVAIL) {
    ...
    if (e.err != FI_ECANCELED || !cmpl.xfc->cancelled) {
      ...
      hlog_fast(
          err, "%s: symbolic flags %s", __func__,
...
static void rcvr_vector_update(fifo_t *ready_for_cxn, rcvr_t *r) {
  bufhdr_t *h;
  vecbuf_t *vb;
  size_t i;
  int rc;
  ...
  while (!fifo_full(r->vec.ready) && !fifo_empty(ready_for_cxn) &&
    ...
    for (i = 0;
         i < arraycount(vb->msg.iov) && (h = fifo_get(ready_for_cxn)) != NULL;
         i++) {
    ...
    (void)fifo_put(r->vec.ready, &vb->hdr);
    hlog_fast(proto_vector, "%s: rcvr %p enqueued vector", __func__, (void *)r);
  }
}

static void rcvr_targets_read(fifo_t *ready_for_terminal, rcvr_t *r) {
  bufhdr_t *h;
  int rc;
...
static loop_control_t rcvr_ack_send(rcvr_t *r) {
...
static void rcvr_cancel(cxn_t *cxn) {
  rcvr_t *r = (rcvr_t *)cxn;
...
static bool rcvr_cancellation_complete(cxn_t *cxn) {
  rcvr_t *r = (rcvr_t *)cxn;

  return fifo_empty(r->progress.posted) && fifo_empty(r->vec.posted);
}

static loop_control_t rcvr_loop(worker_t *w, session_t *s) {
  ...
  if (fifo_eoget(s->ready_for_terminal) && r->cxn.eof.remote &&
      r->cxn.eof.local && fifo_empty(r->vec.posted))
    return loop_end;

  return loop_continue;
}

static loop_control_t xmtr_initial_send(xmtr_t *x) {
...
static loop_control_t xmtr_ack_rx_process(xmtr_t *x, completion_t *cmpl) {
  int rc;
  ...
  rc = fi_av_insert(x->cxn.av, x->ack.msg.addr, 1, &x->cxn.peer_addr, 0, NULL);
  ...
  while (!fifo_full(x->vec.posted)) {
    ...
    rc = buf_mr_reg(global_state.domain, FI_RECV, keysource_next(&x->cxn.keys),
                    &vb->hdr);
...
static loop_control_t xmtr_start(worker_t *w, xmtr_t *x,
                                 fifo_t *ready_for_terminal) {
  x->cxn.started = true;
...
static ssize_t write_fully(const write_fully_params_t p) {
  ssize_t rc;
  ...
  const size_t len =
      minsize(minsize(sumlen.local, sumlen.remote), minsize(p.len, SSIZE_MAX));
...
static bool vecbuf_is_wellformed(vecbuf_t *vb) {
...
static void xmtr_vecbuf_unload(xmtr_t *x) {
  vecbuf_t *vb;
 ...
 * irrecoverable error.
 */
static int xmtr_vector_rx_process(xmtr_t *x, const completion_t *cmpl) {
  vecbuf_t *vb;
 ...
 * any completion occurred, -1 on an irrecoverable error.
 */
static int xmtr_cq_process(xmtr_t *x, fifo_t *ready_for_terminal,
                           bool reregister) {
  struct fi_cq_msg_entry fcmpl;
  completion_t cmpl;
  bufhdr_t *h;
  ...
  case xft_rdma_write:
    ...
    while ((h = fifo_peek(x->wrposted)) != NULL &&
           h->xfc.owner == xfo_program && h->xfc.type == xft_rdma_write &&
           h->xfc.nchildren == 0 && !fifo_full(ready_for_terminal)) {
      int rc;
...
static bufhdr_t *xmtr_buf_split(xmtr_t *x, bufhdr_t *parent, size_t len) {
  bufhdr_t *h;
  fragment_t *f;
 ...
 * Tx buffer.
 */
static loop_control_t xmtr_targets_write(fifo_t *ready_for_cxn, xmtr_t *x) {
  ...
  for (i = 0, total = 0, first_h = last_h = NULL;
    ...
    hlog_fast(write,
              ...
              head->nused, total, maxbytes, x->nriovs,
...
static void xmtr_progress_update(fifo_t *ready_for_cxn, xmtr_t *x) {
  progbuf_t *pb;
...
static void xmtr_cancel(cxn_t *cxn) {
  xmtr_t *x = (xmtr_t *)cxn;
...
static bool xmtr_cancellation_complete(cxn_t *cxn) {
  xmtr_t *x = (xmtr_t *)cxn;
...
static loop_control_t xmtr_loop(worker_t *w, session_t *s) {
  vecbuf_t *vb;
  xmtr_t *x = (xmtr_t *)s->cxn;

  if (xmtr_cq_process(x, s->ready_for_terminal, global_state.reregister) == -1)
    return loop_error;
...
static loop_control_t cxn_loop(worker_t *w, session_t *s) {
  int rc;
  cxn_t *cxn = s->cxn;
...
static loop_control_t session_loop(worker_t *w, session_t *s) {
...
static void sessions_swap(session_t *r, session_t *s) {
  session_t tmp;
...
static void worker_update_load(load_t *load, int nready) {
  ...
  if (load->loops_since_mark < UINT16_MAX) {
    ...
    load->average = (load->average +
                     256 * load->ctxs_serviced_since_mark / (UINT16_MAX + 1)) /
                    2;
...
static bool worker_waitable(worker_t *self) {
...
static void worker_run_loop(worker_t *self) {
  size_t half, i;
  struct epoll_event events[WORKER_SESSIONS_MAX];
  int nevents;
  bool waitable;

  if (global_state.waitfd && (waitable = worker_waitable(self)) &&
  ...
  for (half = 0; half < 2; half++) {
    ...
    for (i = ready_up_to - session_half; i < arraycount(self->session) / 2;
         i++) {
     ...
     */
    for (i = active_up_to - session_half; i < arraycount(self->session) / 2;
         i++) {
      session_t *s = &session_half[i];
    ...
    for (i = ready_from - session_half; i < ready_up_to - session_half; i++) {
      session_t *s;
      cxn_t *c, **cp;
      struct {
        bool cxn_empty;
      ...
      assert(stole || i < ncontexts || !c->sent_first ||
             !fifo_empty(s->ready_for_terminal) || global_state.cancelled);
      ...
      *cp = NULL;
      ...
      if (!global_state.waitfd)
        ;
      else if (epoll_ctl(self->epoll_fd, EPOLL_CTL_DEL, c->cq_wait_fd, NULL) ==
               -1) {
...
static bool worker_is_idle(worker_t *self) {
  ...
  bool idle = (nlocked == 2 && self->nsessions[0] == 0 &&
               self->nsessions[1] == 0 && self_idx + 1 == nworkers_running);
...
static void worker_idle_loop(worker_t *self) {
  const ptrdiff_t self_idx = self - &workers[0];
...
static void worker_stats_log(worker_t *self) {
...
static void *worker_outer_loop(void *arg) {
  worker_t *self = arg;
...
static void worker_paybuflist_destroy(worker_t *w, buflist_t *bl) {
  size_t i;
  int rc;
...
static buflist_t *buflist_create(size_t n) {
...
static buflist_t *worker_paybuflist_create(worker_t *w, uint64_t access) {
...
static void worker_init(worker_t *w) {
  ...
  for (i = 0; i < arraycount(w->mtx); i++) {
    if ((rc = pthread_mutex_init(&w->mtx[i], NULL)) != 0) {
      errx(EXIT_FAILURE, "%s.%d: pthread_mutex_init: %s", __func__, __LINE__,
           strerror(rc));
    }
    if ((rc = fi_poll_open(global_state.domain, &attr, &w->pollset[i])) != 0)
...
static bool worker_launch(worker_t *w) {
  pthread_attr_t attr;
  int i, rc, create_rc;
  ...
  if (global_state.personality == get &&
      (rc = pthread_attr_setaffinity_np(&attr, sizeof(cpuset), &cpuset)) != 0) {
...
static worker_t *worker_create(void) {
  worker_t *w;
  ...
  if (!worker_launch(w)) {
    ...
    if ((w - &workers[0]) + 1 != nworkers_allocated) {
      (void)pthread_mutex_unlock(&workers_mtx);
      errx(EXIT_FAILURE, "%s: worker launch failed irrecoverably", __func__);
    }
...
static void workers_initialize(void) {}

static bool worker_assign_session(worker_t *w, session_t s) {
  size_t half, i;
  int rc;
  ...
  for (half = 0; half < 2; half++) {
    ...
    for (i = 0; i < arraycount(w->session) / 2; i++) {
      session_t *slot = &w->session[half * arraycount(w->session) / 2 + i];
      ...
      *slot = s;
      ...
      atomic_fetch_add_explicit(&w->nsessions[half], 1, memory_order_relaxed);
 ...
 */
static worker_t *workers_assign_session_to_running(session_t s) {
  size_t iplus1;
 ...
 * Caller must hold `workers_mtx`.
 */
static worker_t *workers_assign_session_to_idle(session_t s) {
  size_t i;
 ...
 * Caller must hold `workers_mtx`.
 */
static void workers_wake(worker_t *w) {
  assert(&workers[nworkers_running] == w);
  nworkers_running++;
  pthread_cond_signal(&w->sleep);
}

static worker_t *workers_assign_session(session_t s) {
  worker_t *w;
...
static int workers_join_all(void) {
...
static void cxn_init(cxn_t *c, struct fid_av *av,
                     loop_control_t (*loop)(worker_t *, session_t *),
                     void (*cancel)(cxn_t *),
                     bool (*cancellation_complete)(cxn_t *)) {
  memset(c, 0, sizeof(*c));
...
static void xmtr_memory_init(xmtr_t *x) {
  const size_t txbuflen = strlen(txbuf);
  int rc;

  rc = fi_mr_reg(global_state.domain, &x->initial.msg, sizeof(x->initial.msg),
                 FI_SEND, 0, keysource_next(&global_state.keys), 0,
                 &x->initial.mr, NULL);

  if (rc != 0)
    bailout_for_ofi_ret(rc, "fi_mr_reg");

  rc = fi_mr_reg(global_state.domain, &x->ack.msg, sizeof(x->ack.msg), FI_RECV,
                 0, keysource_next(&global_state.keys), 0, &x->ack.mr, NULL);

  if (rc != 0)
    bailout_for_ofi_ret(rc, "fi_mr_reg");

  rc = fi_mr_reg(global_state.domain, txbuf, txbuflen, FI_WRITE, 0,
                 keysource_next(&global_state.keys), 0, x->payload.mr, NULL);

  if (rc != 0)
    bailout_for_ofi_ret(rc, "fi_mr_reg");
}

static void xmtr_init(xmtr_t *x, struct fid_av *av) {
  ...
  if ((x->vec.posted = fifo_create(64)) == NULL) {
    errx(EXIT_FAILURE, "%s: could not create posted vectors FIFO", __func__);
  }
  if ((x->vec.rcvd = fifo_create(64)) == NULL) {
    errx(EXIT_FAILURE, "%s: could not create received vectors FIFO", __func__);
  }

  if ((x->progress.ready = fifo_create(64)) == NULL) {
    errx(EXIT_FAILURE, "%s: could not create ready progress-buffers FIFO",
         __func__);
  }

  if ((x->progress.posted = fifo_create(64)) == NULL) {
    errx(EXIT_FAILURE, "%s: could not create posted progress-buffers FIFO",
         __func__);
  }

  if ((x->progress.pool = buflist_create(nbufs)) == NULL) {
    errx(EXIT_FAILURE, "%s: could not create progress-message tx buffer pool",
         __func__);
  }

  if ((x->fragment.pool = buflist_create(maxposted)) == NULL) {
    errx(EXIT_FAILURE, "%s: could not create fragment header pool", __func__);
  }
...
static void terminal_init(terminal_t *t,
                          loop_control_t (*trade)(terminal_t *, fifo_t *,
                                                  fifo_t *)) {
  t->trade = trade;
}
...
static void source_init(source_t *s) {
  memset(s, 0, sizeof(*s));
...
static void rcvr_memory_init(rcvr_t *r) {
  int rc;
  ...
  rc = mr_regv_all(global_state.domain, r->initial.iov, r->initial.niovs,
                   ...
                   &global_state.keys, 0, r->initial.mr, r->initial.desc,
                   r->initial.raddr, NULL);
  ...
  rc = mr_regv_all(global_state.domain, r->ack.iov, r->ack.niovs,
                   minsize(2, global_state.mr_maxsegs), FI_RECV, 0,
                   &global_state.keys, 0, r->ack.mr, r->ack.desc, r->ack.raddr,
                   NULL);
...
static void rcvr_init(rcvr_t *r, struct fid_av *av) {
  const size_t nbufs = 16;
  size_t i;
  ...
  if ((r->vec.ready = fifo_create(64)) == NULL) {
    errx(EXIT_FAILURE, "%s: could not create ready vectors FIFO", __func__);
  }
  ...
  if ((r->vec.pool = buflist_create(nbufs)) == NULL) {
    errx(EXIT_FAILURE, "%s: could not create vector-message tx buffer pool",
         __func__);
  }
 ...
 * on endpoint `ep`.
 */
static void post_initial_rx(struct fid_ep *ep, get_session_t *gs) {
  int rc;
...
static get_session_t *get_session_accept(get_state_t *gst) {
...
static put_state_t *put_state_open(void) {
  ...
  rc = fi_av_insert(pst->av, global_state.info->dest_addr, 1, &pst->peer_addr,
                    0, NULL);
...
static get_state_t *get_state_open(void) {
  struct fi_av_attr av_attr = {.type = FI_AV_UNSPEC,
                               .rx_ctx_bits = 0,
                               .count = 0,
                               .ep_per_node = 0,
  ...
  if ((rc = fi_endpoint(global_state.domain, global_state.info, &gst->listen_ep,
                        NULL)) != 0)
    bailout_for_ofi_ret(rc, "fi_endpoint");

  if ((rc = fi_cq_open(global_state.domain, &cq_attr, &gst->listen_cq, NULL)) !=
      0)
    bailout_for_ofi_ret(rc, "fi_cq_open");
...
static void put_session_setup(put_state_t *pst, put_session_t *ps) {
  struct fi_cq_attr cq_attr = {.size = 128,
                               .flags = 0,
                               .format = FI_CQ_FORMAT_MSG,
                               .wait_obj = global_state.waitfd ? FI_WAIT_FD
...
static int count_info(const struct fi_info *first) {
  int count;
...
static const char *personality_to_name(personality_t p) {
  if (p == get)
    return "fget";
  else if (p == put)
    return "fput";
...
static void usage(personality_t personality, const char *progname) {
 ...
 */
static void handle_wakeup(int signum, siginfo_t *info, void *ucontext) {}

/* Handler for SIG{HUP,INT,QUIT,TERM}, used to cancel the program. */
static void handle_cancel(int signum, siginfo_t *info, void *ucontext) {}

static void *await_cancellation(void *arg) {
  sigset_t cancelset;
  size_t i;
  ...
  for (;;) {
    ...
    if ((rc = pthread_kill(global_state.main_thd, SIGUSR2)) != 0) {
      errx(EXIT_FAILURE, "%s: could not signal thread for main thread: %s",
...
static size_t parse_nsessions(const char *s, char flagname) {
  char *end;
  uintmax_t n;
...
int main(int argc, char **argv) {
  ...
  if (strcmp(progname, "fget") == 0) {
    ...
    errx(EXIT_FAILURE, "program personality '%s' is not implemented", progname);
  }
  ...
  while ((opt = getopt(argc, argv, optstring)) != -1) {
    ...
    case 'p':
      ninput = 0;
      (void)sscanf(optarg, "%u - %u%n", &global_state.processors.first,
  ...
  hlog_fast(params, "%ld POSIX I/O vector items maximum", sysconf(_SC_IOV_MAX));
