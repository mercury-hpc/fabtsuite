...
mr_regv_all(struct fid_domain *domain, struct fid_ep *ep,
    const struct iovec *iov, size_t niovs, size_t maxsegs, uint64_t access,
    uint64_t offset, seqsource_t *keys, uint64_t flags, struct fid_mr **mrp,
    ...
    if (ncompleted == -FI_EAVAIL) {
        ...
        if (e.err != FI_ECANCELED || !cmpl.xfc->cancelled) {
            ...
            hlog_fast(err, "%s: completion flags %" PRIx64 " symbolic %s",
                __func__, e.flags,
                completion_flags_to_string(
                    e.flags, flagsbuf[0], sizeof(flagsbuf[0])));
        ...
        for (i = 0; i < maxniovs && (h = fifo_get(ready_for_cxn)) != NULL;
            ...
            if (global_state.reregister &&
                (rc = buf_mr_reg(global_state.domain, r->cxn.ep,
                     payload_access.rx, seqsource_get(&r->cxn.keys), h)) < 0)
...
worker_run_loop(worker_t *self)
{
    ...
    for (half = 0; half < 2; half++) {
        ...
        if (global_state.waitfd) {
            ...
            ncontexts = extract_contexts_for_half(
                session_half, NULL, 0, context, false);
        }
...
cxn_init(cxn_t *c, struct fid_av *av,
    ...
    bool (*cancellation_complete)(cxn_t *), void (*shutdown)(cxn_t *))
{
    memset(c, 0, sizeof(*c));
    c->magic = 0xdeadbeef;
    c->cancel = cancel;
...
xmtr_memory_init(xmtr_t *x)
{
    ...
    rc = fi_mr_reg(global_state.domain, &x->ack.msg, sizeof(x->ack.msg),
        FI_RECV, 0, seqsource_get(&global_state.keys), 0, &x->ack.mr, NULL);
...
rcvr_initial_msg_init(rcvr_t *r, struct fid_ep *listen_ep)
{
    ...
    rc = mr_regv_all(global_state.domain, listen_ep, r->initial.iov,
        r->initial.niovs, minsize(2, global_state.mr_maxsegs), FI_RECV, 0,
        &global_state.keys, 0, r->initial.mr, r->initial.desc, r->initial.raddr,
        NULL);
...
rcvr_shutdown(cxn_t *c)
{
    ...
    if (mr_deregv_all(
            r->ack.niovs, minsize(2, global_state.mr_maxsegs), r->ack.mr) < 0) {
        hlog_fast(err, "%s: could not close ack MR", __func__);
    }
...
put_state_open(void)
{
    ...
    if (rc < 0) {
        bailout_for_ofi_ret(
            rc, "fi_getinfo for peer_addr %s", global_state.peer_addr);
    }
    ...
    rc = fi_av_insert(
        pst->av, addr_info->dest_addr, 1, &pst->peer_addr, 0, NULL);
