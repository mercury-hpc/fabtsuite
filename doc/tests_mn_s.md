# Running Multi-node Test with SLURM.

  A sample [SLURM script](../scripts/fabtrun.slurm) is provided.
...
The above test requires 6 nodes - 2 nodes for `fabtget` and 4 nodes for
`fabtput`.
