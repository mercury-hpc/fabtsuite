...
struct _hlog_ring {
  ...
  uint64_t head_exchanges, heads_erased, tails_zeroed, times_empty, truncations;
  pthread_mutex_t mtx;
  hlog_ring_t *next;
};
 ...
 */
static int hlog_msg_vsnprintf(char *buf, size_t buflen, hlog_msg_t msg,
 ...
 */
static hlog_ring_t *hlog_ring_get(void) {
  hlog_ring_t *r;
  int errnum;
 ...
 * On failure, return < 0.
 */
static int hlog_msg_ring_try_vprintf(hlog_ring_t *r, hlog_msg_t msg,
                                     const char *fmt, va_list ap,
                                     bool *completep) {
  ...
  *completep = true;
  ...
  if (nempty == RING_LENGTH) {
     ...
     */
    const int nwritten =
        hlog_msg_vsnprintf(&r->content[0], RING_LENGTH, msg, fmt, ap);
    if (nwritten < 0)
      return nwritten;
     ...
     */
    const int nwritten =
        hlog_msg_vsnprintf(&r->content[tail], RING_LENGTH - tail, msg, fmt, ap);
    if (nwritten < 0)
      return nwritten;
     ...
     */
    const int nwritten =
        hlog_msg_vsnprintf(&r->content[tail], head - tail, msg, fmt, ap);
    if (nwritten < 0)
      return nwritten;
 ...
 */
static void hlog_ring_dump(hlog_ring_t *r) {
  assert(r->consumer <= r->producer);
  ...
  fprintf(stderr,
          ...
          r->heads_erased, r->tails_zeroed, r->times_empty, r->truncations);
 ...
 * error stream.
 */
static void hlog_ring_dump_all(void) {
  hlog_ring_t *first, *last, *r;

  for (first = hlog_ring_head, last = NULL; first != last;
 ...
 */
void hlog_abort(const char *reason) {
  hlog_ring_dump_all();
  fprintf(stderr, "%s\n", reason);
  fflush(stderr);
  abort();
 ...
 * failure.
 */
static int hlog_msg_ring_vprintf(hlog_msg_t msg, const char *fmt, va_list ap) {
  bool complete;
  int nwritten;
  ...
  do {
    ...
    va_copy(ap_copy, ap);
    nwritten = hlog_msg_ring_try_vprintf(r, msg, fmt, ap_copy, &complete);
    va_end(ap_copy);
    if (nwritten < 0)
      break;
  } while (!complete);
...
static void hlog_outlet_state_init(void) {
...
static void hlog_output_state_init(void) {
  const char *setting;
  ...
  if (strcmp(setting, "stderr") == 0)
    ...
    warn("%s: unknown hlog output \"%s\" selected", __func__, setting);
}
...
static void hlog_msg_vfprintf(FILE *stream, hlog_msg_t msg, const char *fmt,
                              va_list ap) {
  if (msg.prefix) {
...
static hlog_msg_t hlog_msg_init(const hlog_outlet_t *ls, int errnum) {
  ...
  return (hlog_msg_t){.errnum = errnum,
                      .elapsed = elapsed,
                      .prefix = (ls != NULL) ? ls->ls_prefix : true,
                      .suffix = (ls != NULL) ? ls->ls_suffix : true};
}

static void vhlog_msg(hlog_msg_t msg, const char *fmt, va_list ap) {
  switch (hlog_out) {
...
void vhlog(const hlog_outlet_t *ls, const char *fmt, va_list ap) {
  vhlog_msg(hlog_msg_init(ls, 0), fmt, ap);
}

void vhlog_err(int status, const char *fmt, va_list ap) {
  vhlog_msg(hlog_msg_init(NULL, errno), fmt, ap);
  exit(status);
}

void vhlog_errx(int status, const char *fmt, va_list ap) {
  vhlog_msg(hlog_msg_init(NULL, 0), fmt, ap);
  exit(status);
}

void vhlog_warn(const char *fmt, va_list ap) {
  vhlog_msg(hlog_msg_init(NULL, errno), fmt, ap);
}

void vhlog_warnx(const char *fmt, va_list ap) {
  vhlog_msg(hlog_msg_init(NULL, 0), fmt, ap);
}

void hlog_err(int status, const char *fmt, ...) {
  va_list ap;
...
void hlog_errx(int status, const char *fmt, ...) {
  va_list ap;
...
void hlog_warnx(const char *fmt, ...) {
  va_list ap;
...
struct hlog_outlet *hlog_outlet_find_active(struct hlog_outlet *ls0) {
...
void hlog_always(const hlog_outlet_t *ls, const char *fmt, ...) {
  va_list ap;
...
void hlog_impl(struct hlog_outlet *ls0, const char *fmt, ...) {
  struct hlog_outlet *ls;
  va_list ap;
...
static void hlog_outlet_reset_all(void) {
  struct hlog_outlet *ls;
...
struct hlog_outlet *hlog_outlet_lookup(const char *name) {
  struct hlog_outlet *ls;
...
static struct hlog_outlet *hlog_outlet_create(const char *name) {
  struct hlog_outlet *ls;
...
static void hlog_outlet_destroy(struct hlog_outlet *ls) {
  /*LINTED*/
  if (ls->ls_name0 != NULL)
    free(ls->ls_name0);
  free(ls);
...
int hlog_set_output(hlog_output_t out) {
  switch (out) {
  case HLOG_OUTPUT_STDERR:
  case HLOG_OUTPUT_STDOUT:
  case HLOG_OUTPUT_RING:
...
int hlog_set_state(const char *name, hlog_outlet_state_t state,
                   bool rendezvous) {
  struct hlog_outlet *ls;
  errno = 0;
...
void hlog_outlet_register(struct hlog_outlet *ls_arg) {
  struct hlog_outlet *ls;
