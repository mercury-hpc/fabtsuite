...
void
hlog_init(void) hlog_constructor;
static void
...
static int
hlog_msg_vsnprintf(
    char *buf, size_t buflen, hlog_msg_t msg, const char *fmt, va_list ap)
{
 ...
 */
static int
hlog_msg_ring_try_vprintf(hlog_ring_t *r, hlog_msg_t msg, const char *fmt,
    va_list ap, bool *completep)
{
...
hlog_outlet_state_init(void)
{
    ...
    while ((item = strsep(&iter, " ,")) != NULL) {
        ...
        if (strcmp(val, "on") == 0 || strcmp(val, "yes") == 0)
            ...
            warnx("%s: bad HLOG value \"%s\" in item \"%s\"", __func__, val,
                item);
            continue;
        }
        ...
        if (hlog_set_state(key, state, true) == -1) {
            warn(
                "%s: could not set state for HLOG item \"%s\"", __func__, item);
        }
    }
...
hlog_msg_vfprintf(FILE *stream, hlog_msg_t msg, const char *fmt, va_list ap)
{
    ...
    if (msg.errnum != 0) {
        (void) fprintf(
            stream, ": %s%s", strerror(msg.errnum), msg.suffix ? "\n" : "");
    } else if (msg.suffix)
        (void) fputc('\n', stream);
}
